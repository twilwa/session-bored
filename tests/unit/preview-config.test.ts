// ABOUTME: Verifies a pull request preview binds its own D1 database, R2 bucket, and origin.
// ABOUTME: Keeps a preview with incomplete resource names from falling back to production bindings.
...
const bindings = {
  ...
  bucketName: "session-bored-pr-172-files",
  previewUrl: "https://pr-172-session-bored.techwilliams-warren.workers.dev",
};

describe("preview configuration", () => {
  it("points the preview at the pull request's own database, bucket, and origin", () => {
    ...
    expect(config.r2_buckets[0]).toMatchObject({
      binding: "FILES",
      bucket_name: bindings.bucketName,
    });
    expect(config.r2_buckets[0]?.bucket_name).not.toBe("session-bored-files");
  ...
  it("refuses to build a config that would deploy against another database", () => {
    ...
    expect(() => buildPreviewConfig(productionConfig, { ...bindings, bucketName: undefined }))
      .toThrow(/bucketName/);
